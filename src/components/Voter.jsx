...
class Voter extends Component {
  ...
  handleVoteClick = userVote => {
    if (this.props.article_id) {
      api.patchVotes("articles", this.props.article_id, userVote).catch(() => {
        this.setState({ err: true });
      });
    } else {
      api.patchVotes("comments", this.props.comment_id, userVote).catch(() => {
        this.setState({ err: true });
      });
    }
    this.setState(currentState => {
      return {
        optimisticVote: currentState.optimisticVote + userVote
      };
    });
  };
  ...
  render() {
    ...
    return (
      <section className="voter">
        <p className="voter--title">
          Votes: {optimisticVote + this.props.votes}
        </p>
        <button
          className={
            btnHasBeenClicked ? "voter--btn-inactive voter--btn" : "voter--btn"
          }
          id="upvote"
          onClick={clickEvent => {
        ...
        {btnHasBeenClicked && err && (
          <p className="voter--errorMsg">
            Sorry, votes can't be added right now.
          </p>
        )}
      </section>
    );
